...
class CController extends BaseController {
	...
	#######################################################
	##                                                   ##
	##      GET /c/igames/{func}/{action}/{value}        ##
	##                                                   ##
	#######################################################
	public function getIgames($func, $action, $value)
	{
		switch ($func) 
		{
			case 'account':
				switch ($action) {
					case 'accountid':
						$this->get_igames_account_accountid($value);
						break;
				}
				break;
	...
	public function get_igames_account_accountid($id)
	{
		$accountid = ($id * 1000);
		...
		if($account->identities != null)
		{
			$body_identities = array(
				'strife.'.$account->account_id => array(
					...
					'ident_id' => $account->identities->ident_id,
				)
			);
		}
		...
		$this->response['body'] = array(
				...
				'account_id' => $account->identities->ident_id,
			);
	}
	...
	#######################################################
	##                                                   ##
	##      POST /c/igames/{func}/{action}/{value}       ##
	##                                                   ##
	#######################################################
	public function postIgames($func, $action = null, $value = null)
	{
		switch ($func) 
		{
			case 'asiasoftSession':
				return $this->post_igames_asiasoftSession(
					Input::get('id'), 
					Input::get('password'), 
	...
	public function post_igames_asiasoftSession($id, $password, $identities, $akey = null)
	{
		$account = DB::table('account')
					->where('email', $id)
					->first();
	...
	#######################################################
	##                                                   ##
	##      GET /c/strife/{func}/{action}/{value}        ##
	##                                                   ##
	#######################################################
	public function getStrife($func, $action, $value)
	{
		switch ($func) {
			case 'ident':
				switch ($action) {
					case 'identid':
						return $this->get_strife_ident_identid($value);
						break;
					case 'playerResources':
						return $this->get_strife_ident_playerResources($value);
						break;
				}
				break;
			...
			case 'profile':
				switch ($action) {
					case 'identid':
						return $this->get_strife_profile_identid($value);
						break;
				}
				break;
	...
	public function get_strife_ident_identid($id)
	{
		$accountid = ($id * 1000);
		$account = DB::table('account')
					->where('account_id', $accountid)
	...
	public function get_strife_ident_playerResources($id)
	{
		$accountid = ($id * 1000);
		$account = DB::table('account')
					->where('account_id', $accountid)
		...
		$this->identIncrement = explode('.', $id)[0];
		$this->shard = explode('.', $id)[1];
		$this->response['body'] = array(
			'identIncrement' => $this->identIncrement,
			'shard' => $this->shard,
	...
	public function get_strife_profile_identid($id)
	{
		$accountid = ($id * 1000);
		$account = DB::table('account')
					->where('account_id', $accountid)
		...
		$this->response['body'] = array(
			...
			'clientAccountIcons' => array(
				'clientAccountIcons' => array(
					$account->identities->ident_id => array(
						'active' => 1,
	...
	#######################################################
	##                                                   ##
	##      POST /c/strife/{func}/{action}/{value}       ##
	##                                                   ##
	#######################################################
	public function postStrife($func, $action, $value)
	{
		switch ($func) {
			case 'ident':
				switch ($action) {
					case 'accountid':
						return $this->post_strife_ident_accountid($value);
						break;
					case 'identid':
						return $this->post_strife_ident_identid($value);
						break;
				}
				break;
	...
	public function post_strife_ident_accountid($id)
	{
		$accountid = ($id * 1000);
		$account = DB::table('account')
					->where('account_id', $accountid)
	...
	public function post_strife_ident_identid($id)
	{
		$accountid = ($id * 1000);
		$account = DB::table('account')
					->where('account_id', $accountid)
		...
		switch ($action) {
			case 'setTutorialProgress':
				{
					DB::table('account_identity')
			            ->where('account_id', $accountid)
			            ->update(array('tutorialProgress' => Input::get('progress')));
			        ...
			        $this->response['body'] = array(
						...
						'canCraft' => $account->identities->canCraft,
						'canPlayRanked' => $account->identities->canPlayRanked,
						'lanBoosts' => $account->identities->lanBoosts,
						'identStats' => array(
							'stats' => array(
								'matchmakingIdentStats' => array(
								),
	...
	public function post_strife_pet_identid($id)
	{
